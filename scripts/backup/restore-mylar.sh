...
echo "🔄 Mylar Restore Script"
echo "======================="
echo ""

# Configuration
BACKUP_BASE_DIR="/backup/mylar"
...
for backup_dir in $BACKUPS; do
    ...
    backup_file="${backup_dir}/mylar-config.tar.gz"
...
BACKUP_FILE="${selected_backup}/mylar-config.tar.gz"
...
echo ""
echo "⚠️  WARNING: This will restore Mylar configuration from backup: ${BACKUP_NAME}"
...
CURRENT_BACKUP_DIR="/tmp/mylar-current-backup-$(date +%Y%m%d_%H%M%S)"
mkdir -p "$CURRENT_BACKUP_DIR"
if [ "$(ls -A $RESTORE_TARGET 2>/dev/null)" ]; then
    tar -czf "${CURRENT_BACKUP_DIR}/mylar-config-current.tar.gz" -C "$RESTORE_TARGET" . || true
...
# Restore from backup
echo "📦 Restoring Mylar configuration from ${BACKUP_NAME}..."
tar -xzf "$BACKUP_FILE" -C "$RESTORE_TARGET"

# Verify restoration
if [ $? -eq 0 ]; then
    echo "✅ Mylar configuration restored successfully!"
    echo ""
    ...
    echo ""
    echo "⚠️  Please restart the Mylar pod for changes to take effect:"
    echo "   kubectl rollout restart deployment/mylar -n media"
else
    echo "❌ Error: Restore failed!"
    echo "⚠️  Attempting to restore previous configuration..."
    if [ -f "${CURRENT_BACKUP_DIR}/mylar-config-current.tar.gz" ]; then
        tar -xzf "${CURRENT_BACKUP_DIR}/mylar-config-current.tar.gz" -C "$RESTORE_TARGET"
